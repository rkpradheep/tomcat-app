...
public class CustomErrorReportValve extends JsonErrorReportValve
{
	...
	static
	{
		try
		{
			BufferedReader bufferedReader = new BufferedReader(new FileReader(System.getenv("TOMCAT_APP") + "/build/conf/errorPage.html"));
	...
	protected void report(Request request, Response response, Throwable throwable)
	{
		try
		{
			...
			{
				String redirectUri = request.getRequestURI().replaceAll("(/*)$", "");
				response.sendRedirect(redirectUri.equals("/")? "/" : redirectUri);
				return;
			}
