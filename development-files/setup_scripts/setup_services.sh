...
sudo cp $TOMCAT_APP/tomcat.service /etc/systemd/system
if test "$DB_SERVER" = "mysql" ; then
  sudo cp $TOMCAT_APP/mysql.service /etc/systemd/system
  sudo sh -c "sed -i 's|mariadb.service|mysql.service|' /etc/systemd/system/tomcat.service"
else
  sudo cp $TOMCAT_APP/mysql.service /etc/systemd/system
fi

sudo sh -c "sed -i 's|home_ph|${TOMCAT_APP}|' /etc/systemd/system/tomcat.service"
